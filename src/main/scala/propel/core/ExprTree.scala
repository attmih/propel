...
object ExprTree {
    ...
    /** Lambda represents a function body with a single bound variable.
      ...
      * {{{
      * Lambda (a => Lambda (b => Apply (Apply (+, a), b)))
      * }}}
      *
      * As you can see from the above the body of a Lambda node is defined as a function.
      * The alternative would be to define a specific variable name and then refer to that
      * in the body but that could cause unwanted variable capture during evaluation.
      *
      * Unique variable names avoid capture but ensuring uniqueness is difficult in the local
      * context of an expression tree node. Instead we define the body as a function so that
      * the processing logic can pass in a unique variable name after running a global analysis.
      *
    ...
    /** Allows the logic to branch out based on the value of a reference expression.
      *
      * This is similar pattern matching in functional languages and SQL's case statements. It can also
